import { A as AstroError, U as UnknownContentCollectionError, c as createComponent, R as RenderUndefinedEntryError, u as unescapeHTML, a as renderTemplate, g as escape, h as renderUniqueStylesheet, i as renderScriptElement, j as createHeadAndContent, r as renderComponent, b as createAstro, m as maybeRenderHead } from '../chunks/astro/server_Cs1_6WaH.mjs';
...
async function getSeedQuery({ uri }) {
...
async function getTemplate(seedNode, templates) {
  if (!seedNode) {
    return void 0;
  }
...
class ImmutableDataStore {
  ...
  static async fromModule() {
    try {
      const data = await import('../chunks/_astro_data-layer-content_DeGJJA8Z.mjs');
...
async function updateImageReferencesInBody(html, fileName) {
  ...
  const { getImage } = await import('../chunks/_astro_assets_fg4Pzi2i.mjs').then(n => n._);
...
const $$ = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const template = await getTemplate(
    data.nodeByUri,
    availableTemplates.map((template2) => template2.data)
  );
  Astro2.locals.uri = uri;
  if (template) {
    return Astro2.rewrite(template?.path);
  }
  return renderTemplate`${maybeRenderHead()}<div>Something is wrong if you're seeing this</div> <pre>${JSON.stringify(template, null, 2)}</pre> <pre>${JSON.stringify(data, null, 2)}</pre>`;
}, "/Users/alex.moon/code/moonmeister/gurugoes.net/src/pages/[...uri].astro", void 0);

const $$file = "/Users/alex.moon/code/moonmeister/gurugoes.net/src/pages/[...uri].astro";
const $$url = "/[...uri]";
