...
const MobileMenuItem = ({ to, children, ...rest }) => (
  <div className="px-2 pt-2 pb-3" role="none">
    <Link to={to} className="block px-3 py-2 hover:bg-gray-50" role="menuitem" {...rest}>{children}</Link>
  </div>
)
          ...
          <div className="flex items-center flex-1 md:absolute md:inset-y-0 md:left-0">
            <div className="flex items-center justify-between w-full md:w-auto">
              <div className="-mr-2 flex items-center md:hidden">
                <button type="button" onClick={() => setIsOpen(!isOpen)} className="bg-gray-50 rounded-md p-2 inline-flex items-center justify-center text-gray-400 hover:text-gray-500 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-indigo-500" id="main-menu" aria-haspopup="true">
          ...
          <div className="hidden md:flex md:space-x-10">
...

            {
              categories?.map(({ name, uri }) => (
                currentCategory !== name ? <Link to={uri} role="menuitem">{name}</Link> : null
              ))
            }
          </div>
        </nav>
        ...
        <div className="absolute top-0 inset-x-0 p-2 transition transform origin-top-right md:hidden">
          <div className="rounded-lg shadow-md bg-white ring-1 ring-black ring-opacity-5 overflow-hidden">
            ...
            <div role="menu" aria-orientation="vertical" aria-labelledby="main-menu">
              {currentCategory !== "all" ? <MobileMenuItem to={"/"}>All</MobileMenuItem> : null}

              {categories?.map(({ name, uri }) => (
                currentCategory === name ? null :
                  <MobileMenuItem to={uri} onClick={() => setIsOpen(!isOpen)}>{name}</MobileMenuItem>
              ))}
            </div>
          </div>
        </div>
